...
def postgres_to_snowflake_etl():
    ...
    for table_name in table_names:  
        ...
        def get_max_primary_key(table_name: str):  # 1 task
            with SnowflakeHook(snowflake_conn_id='snowflake').get_conn() as conn:  # conectando no snowflae
        ...
        def load_incremental_data(table_name: str, max_id: int):  # 2 task
            with PostgresHook(postgres_conn_id='postgres').get_conn() as pg_conn:  # conectando no postgres
